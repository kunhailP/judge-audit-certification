...
def main():
    ...
    for (c, j, e), g in d.groupby(["collection", "judge", "eps"]):
        ...
        for m, gm in g0.groupby("method"):
            if m == REF:
                continue
            gm = gm.set_index("draw"); vr = (gm.v_pilot / ref.v_pilot).dropna(); sav = (1 - vr) * share.reindex(vr.index); sav2 = (1 - vr) * share2.reindex(vr.index)
    ...
    for (c, j, e), g in t.groupby(["collection", "judge", "eps"]):
        ...
        lines.append(f"## {c} / judge={j} / eps={e}  (N={int(g.N.iloc[0])}, pilot={g.pilot.iloc[0]:.0f} labels, predicted post-pilot labels of weighted: v1 {g.L_post_pred_med.iloc[0]:.0f}, v2 {g.L_post_pred_v2_med.iloc[0]:.0f})")
        for m in ["uniform", "uniform_nz", "strat_pilot", "weighted_cv", "weighted_cvl", "active_cv", "active_judge", "ai_calib", "ai_resid", "ai_robust_0.3"]:
            if m in g.index:
                ...
                dec2 = {1: "RECOMMEND", 0: "do not recommend", -1: ""}[int(r.decision_v2)]
                lines.append(f"- {m:14s} predicted cost ratio {r.vr_med:.2f} [{r.vr_q25:.2f}, {r.vr_q75:.2f}]; total saving v1 {r.saving_pred_med:+.3f} (pilots>tau {r.frac_recommend:.2f}) {dec}; v2 {r.saving_pred_v2_med:+.3f} (pilots>tau {r.frac_recommend_v2:.2f}) {dec2}")
