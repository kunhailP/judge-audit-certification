...
(05_results/planner_v2/{stack}_llm_pop_{t,bet_wor}_ntr20_v3): every split arm is evaluated until IT terminates.
...
the whole population is labelled" is visible. Writes 05_results/unified/TABLE_exact_v3.csv and TABLE_exact_v3.tex.
"""
import os
import numpy as np, pandas as pd

HUB = os.path.dirname(os.path.dirname(os.path.abspath(__file__))); R = os.path.join(HUB, "05_results", "planner_v2")
...
def main():
    ...
    t = pd.DataFrame(rows); out = os.path.join(HUB, "05_results", "unified"); os.makedirs(out, exist_ok=True)
