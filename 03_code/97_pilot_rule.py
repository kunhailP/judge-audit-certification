...
Output: 04_results/unified/PILOT_RULE_eps{eps}.csv and a printed table.
"""
...
def main():
    ...
    if "sb2_pilot" not in d or d.sb2_pilot.isna().all():
        ...
        if pf:
            ...
            extra = [c for c in pr.columns if c in ("sb2_pilot", "n_train", "slack_xfit") or c.startswith(("slack_c", "sb2_c", "v_pilot_c", "v_pilot_ref_c"))]
    ...
    for (c, j), g in d.groupby(["collection", "judge"]):
        ...
        for m, gm in g0.groupby("method"):
            ...
            L4 = pd.Series([cp.l_post_v2(v, sb, sl, z, int(g.n_queries.iloc[0]), n_tr, b_ref) for v, sb, sl in zip(ref.v_pilot, ref.sb2_pilot, ref.slack_xfit)], index=ref.index) if "slack_xfit" in ref else L2 * np.nan
            share4 = (L4 / (L4 + P)).reindex(vr.index); pred_saving_v4 = (1 - vr) * share4
            ...
            rows.append(dict(collection=c, judge=j, eps=a.eps, method=m, pilot=P, J50_ref=J[REF], J50=J[m], post_pilot_share=share,
                             ...
                             L_post_pred_v4_med=float(L4.median()), saving_pred_v4_med=float(pred_saving_v4.median()),
                             frac_draws_recommend_v4=float((pred_saving_v4 > a.tau).mean()), slack_xfit_med=float(ref.slack_xfit.median()) if "slack_xfit" in ref else np.nan,
    ...
    if len(t):
        ...
        print(f"fully pilot-based, v4 predictor (v2 with cross-fitted slack): corr = {np.corrcoef(t.saving_pred_v4_med, t.saving_obs)[0,1]:.3f}; MAE = {np.abs(t.saving_pred_v4_med - t.saving_obs).mean():.3f}")
        print("post-pilot labels of weighted: realised vs v1 vs v2 vs v3 vs v4 (medians): " + "; ".join(f"{r.collection}/{r.judge} {r.L_post_obs:.0f} vs {r.L_post_pred_med:.0f} vs {r.L_post_pred_v2_med:.0f} vs {r.L_post_pred_v3_med:.0f} vs {r.L_post_pred_v4_med:.0f}" for r in u.itertuples()))
        print("pilot slack: plain vs cross-fitted vs true (medians): " + "; ".join(f"{r.collection}/{r.judge} {r.slack_pilot_med:.4f} vs {r.slack_xfit_med:.4f} vs {r.slack_true_med:.4f}" for r in u.itertuples()))
        u = t.drop_duplicates(["collection", "judge"])
        ...
        for col, lab in [("frac_draws_recommend", "share from realised J50"), ("frac_draws_recommend_pilot", "fully pilot-based v1"), ("frac_draws_recommend_v2", "fully pilot-based v2"), ("frac_draws_recommend_v3", "fully pilot-based v3"), ("frac_draws_recommend_v4", "fully pilot-based v4")]:
