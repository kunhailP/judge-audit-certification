...
# committed BEFORE step 4 runs the audit. Run from 03_code. Requires the pools bundle (training pools) and /workspace/neuclir.
set -euo pipefail
...
if [ "$STEP" = predict ] || [ "$STEP" = all ]; then
  ...
  cd .. && for judge in llm rr inv; do python3 03_code/98_heldout_predict.py --glob "04_results/sampling_baselines/baselines_neuclir_${judge}_nc_b*_predict.csv" --tag neuclir_${judge} > $LOG/lock_${judge}.txt
    python3 03_code/98_heldout_predict.py --glob "04_results/sampling_baselines/baselines_neuclir_${judge}_nc10_b*_predict.csv" --tag neuclir_${judge}_pilot10 > $LOG/lock_${judge}_pilot10.txt; done; cd 03_code
  echo "[predict] done -> commit 04_results/heldout/PREDICTIONS_neuclir_* and the *_predict.csv files, THEN run: ./RUN_NEUCLIR.sh audit"
fi
if [ "$STEP" = audit ]; then
  for judge in llm rr inv; do for nt in 20 10; do tag=$([ $nt = 20 ] && echo _nc || echo _nc10); for B in $BUDGETS; do
    while [ "$(jobs -rp | wc -l)" -ge 31 ]; do sleep 2; done
  ...
  cd .. && for judge in llm rr inv; do for tag in nc nc10; do for eps in 0.02 0.01; do python3 03_code/97_pilot_rule.py --eps $eps --glob "04_results/sampling_baselines/baselines_neuclir_${judge}_${tag}_*_draws.csv" > $LOG/score_${judge}_${tag}_eps$eps.txt
    mv 04_results/unified/PILOT_RULE_eps$eps.csv 04_results/heldout/PILOT_RULE_neuclir_${judge}_${tag}_eps$eps.csv; mv 04_results/unified/PILOT_RULE_single_eps$eps.csv 04_results/heldout/PILOT_RULE_single_neuclir_${judge}_${tag}_eps$eps.csv 2>/dev/null || true; done; done; done
  echo "[audit] done -> scores in $LOG/score_*.txt and 04_results/heldout/"
fi
