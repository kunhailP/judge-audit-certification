...
Writes 05_paper/TABLES_v0.1.md and 04_results/unified/J50.csv.
"""
import glob, json, os
import numpy as np, pandas as pd

HUB = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
R = os.path.join(HUB, "04_results"); OUT = os.path.join(R, "unified"); os.makedirs(OUT, exist_ok=True)
...
for block, g in df.groupby("block", sort=True):
    ...
    for (c, e), gg in g.groupby(["collection", "eps"]):
        ...
        lines.append("")
open(os.path.join(HUB, "05_paper", "TABLES_v0.1.md"), "w").write("\n".join(lines))
